...
eval(function (p, a, c, k, e, d) {
    ...
    while (c--) {
        if (k[c]) {
            p = p[_0xf6db[4]](new RegExp(_0xf6db[7] + e(c) + _0xf6db[7], _0xf6db[8]), k[c]);
        }
        ;
    }
    ;
...
Redactor.prototype = {
    ...
    _insertLink: function (a, text, link) {
        ...
        if (text !== '') {
            if (this.insert_link_node) {
                ...
                if ($.browser.msie) {
                    $('#span' + this.spanid).replaceWith(a);
                    this.syncCode();
                }
                else {
    ...
    showFile: function () {
        ...
        var handler = $.proxy(function () {
            ...
            this.uploadInit('redactor_file', {
                auto: true, url: this.opts.fileUpload, success: $.proxy(function (data) {
...
(function ($) {
    $.event.special.textselect = {
        setup: function (data, namespaces) {
            ...
            var text = '';
            if (window.getSelection) text = window.getSelection(); else if (document.getSelection) text = document.getSelection(); else if (document.selection) text = document.selection.createRange().text;
            return text
        }
    };
