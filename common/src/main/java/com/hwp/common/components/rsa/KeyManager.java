package com.hwp.common.components.rsa;

import com.hwp.common.util.Base64Utils;
...
public class KeyManager {
    ...
    public static void genKeyPair(Map<String, String> params) throws Exception {
        ...
        if (url.indexOf("vjinke.com") >= 0) {
            keyToFile(Base64Utils.encode(publicKey.getEncoded()), shortName, "pubKey.pfx");
            keyToFile(Base64Utils.encode(privateKey.getEncoded()), shortName, "priKey.cer");
            // keyToFile(
            // Base64Utils.encode(publicKey.getEncoded()),"pubKey.key");
            // keyToFile(
            // Base64Utils.encode(privateKey.getEncoded()),"priKey.key");
        } else {
            keyToFile(Base64Utils.encode(publicKey.getEncoded()), shortName, "pubKey" + ptrId + ".pfx");
            keyToFile(Base64Utils.encode(privateKey.getEncoded()), shortName, "priKey" + ptrId + ".cer");
            // keyToFile(
    ...
    public static void keyToFile(String key, String shortName, String keyFileName) throws Exception {
        if (key != null && !"".equals(key)) {
            ...
            // 获取项目应用的路径
            String filePath = ApplicationPath.getRootPath();
            filePath = KEY_FILE_PATH + File.separator + "keysManage";
            //
    ...
    public static void main(String[] args) throws Exception {
        ...
        map.put("seed", "2ede7022d813b90bd99f4a1bce129786");
        map.put("ptrId", "JKGF");
        map.put("url", "www.jkgf.com");
        map.put("shortName", "JKGF");
