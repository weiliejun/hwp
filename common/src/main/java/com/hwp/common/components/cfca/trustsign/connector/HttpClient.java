...
public class HttpClient {
    ...
    public static final String DEFAULT_SSL_PROTOCOL = "TLSv1.1";// TSL1.1通道
...
//                fileInputStream = new FileInputStream(keyStorePath);
                fileInputStream = this.getClass().getResourceAsStream(keyStorePath);
...
//                fileInputStream = new FileInputStream(trustStorePath);
                fileInputStream = this.getClass().getResourceAsStream(trustStorePath);
    ...
    public int send(HttpURLConnection connection, byte[] requestData, File file, String signature) throws IOException {
        if (requestData != null) {
            ...
            if (file != null) {
                ...
                // byte[] gzipBytes = GZipUtil.compress(new
    ...
    public int send(HttpURLConnection connection, byte[] requestData, OutputStream out, String signature) throws IOException {
        if (requestData != null) {
            String BOUNDARY = java.util.UUID.randomUUID().toString();
            connection.setRequestProperty("Connection", "keep-alive");
            connection.setRequestProperty("Charsert", "UTF-8");
            ...
            if (out != null) {
                ...
                sb1.append("\r\n");
                // name是post中传参的键 filename是文件的名称
                sb1.append("Content-Disposition: form-data; name=\"contractFile\"; filename=\"" + "借款合同" + "\"" + "\r\n");
                sb1.append("Content-Type: application/octet-stream; charset=" + "--" + "\r\n");
                sb1.append("\r\n");
                outputStream.write(sb1.toString().getBytes());
                ...
                // ByteArrayInputStream is = new
                // ByteArrayInputStream(gzipBytes);
                ByteArrayOutputStream outArr = (ByteArrayOutputStream) out;
                outputStream.write(outArr.toByteArray());
                outArr.close();
                outputStream.write("\r\n".getBytes());
            }
